...
# locals {
#   subnet_map = module.Vnet.vnet_subnets_name_id
# }
...
resource "azurerm_storage_account" "st" {
  ...
  blob_properties {
    ...
    cors_rule {
      ...
      allowed_origins    = ["https://*.eximius.ai"]
      exposed_headers    = [""]
  ...
  network_rules {
    default_action             = "Deny"
    ip_rules                   = var.st_allowed_ip_addresses
    bypass                     = ["AzureServices"]
  }
  tags = local.tags
}
...
resource "azurerm_private_endpoint" "storage_pe" {
  ...
  depends_on = [ module.Vnet, azurerm_storage_account.st, azurerm_private_dns_zone.storage ]
}
...
module "aks" {
  ...
  agents_min_count                     = 1
  agents_max_count                     = 2
  agents_pool_name                     = var.agent_name
